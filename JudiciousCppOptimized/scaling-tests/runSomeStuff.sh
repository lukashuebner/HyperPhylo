...
i10pc127|i10pc132)
        ...
        exclusive ./runTests.py --algorithm sparse --machine-id `hostname` --nthreads 1 2 4 8 9 10 12 16 17 18 20 24 25 26 28 32 --param MSACONVERTER_BIN=`hostname` --param JUDICIOUS_BIN=`hostname` --param K=50 --scaling strong --param THREAD_PINNING=True $DRY_RUN | tee `hostname`-testResults.csv
        ;;
i10pc129)
